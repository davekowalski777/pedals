...
class PedalListView(ListView):
    ...
    def get_queryset(self):
        queryset = Pedal.objects.all().select_related('manufacturer', 'pedal_type', 'author')
...
class PedalDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Get related pedals (same type or manufacturer, excluding current pedal)
        related_pedals = Pedal.objects.filter(
            Q(pedal_type=pedal.pedal_type) | Q(manufacturer=pedal.manufacturer)
        ).exclude(id=pedal.id).order_by('-my_rating')[:3]
        
        context['related_pedals'] = related_pedals
